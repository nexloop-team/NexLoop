...
const steps = [
  {
    ...
    description:
      "We audit your current setup and identify precisely where intelligent automation and AI systems can generate the highest impact — from lead capture to CRM automation.",
...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="relative py-16 sm:py-28 px-4 sm:px-6">
      <div className="relative max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="mt-3 sm:mt-4 text-white/45 text-sm sm:text-base max-w-md mx-auto">
        ...
        <div className="flex flex-col gap-3 sm:gap-4">
          {steps.map((step, i) => (
            <motion.div
              ...
              className="group glass-card p-5 sm:p-8 flex flex-row items-start gap-4 sm:gap-6"
            >
              {/* Number — smaller on mobile */}
              <span
                className="text-4xl sm:text-6xl font-bold leading-none select-none shrink-0 tabular-nums w-10 sm:w-auto"
                style={{ fontVariantNumeric: "tabular-nums", color: "rgba(124,58,237,0.2)" }}
              >
                {step.number}
              </span>

              {/* Icon — hidden on very small to keep layout clean */}
              <div className="hidden sm:flex w-12 h-12 rounded-xl bg-purple-600/15 border border-purple-500/20 items-center justify-center shrink-0 group-hover:bg-purple-600/25 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 sm:hidden mb-2">
                  <div className="w-7 h-7 rounded-lg bg-purple-600/15 border border-purple-500/20 flex items-center justify-center shrink-0">
                    <step.icon size={14} className="text-purple-400" />
                  </div>
                  <h3 className="text-base font-bold text-white tracking-tight leading-tight">
                    {step.title}
                  </h3>
                </div>
                <h3 className="hidden sm:block text-lg sm:text-xl font-bold text-white mb-1.5 tracking-tight">
                  {step.title}
                </h3>
                <p className="text-white/40 text-sm leading-relaxed">{step.description}</p>
                <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-3 sm:mt-4">
                  {step.chips.map((chip) => (
                    <span key={chip} className="tag-chip text-xs py-1 px-2 sm:px-3">{chip}</span>
                  ))}
                </div>
              </div>
            </motion.div>
