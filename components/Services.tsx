...
const features = [
  ...
  },
  {
    ...
    description:
      "Intelligent automation pipelines that work while you sleep — capturing leads, sending follow-ups, syncing your CRM, and triggering next steps without any manual effort.",
...
function ChatMockup() {
  return (
    <div className="mockup-shell h-full min-h-[160px] p-3 sm:p-4 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-2 p-1 sm:p-2">
        <div className="flex items-start gap-2">
          ...
          <div className="bg-white/5 border border-white/8 rounded-xl rounded-tl-sm px-3 py-2">
            ...
            <p className="text-[10px] text-white/30 mt-1 hidden sm:block">I can qualify leads, answer FAQs, or book a call — 24/7</p>
          </div>
        </div>
...
function DashboardMockup() {
  const items = [
    ...
    { label: "WhatsApp automation", sub: "38 messages sent", color: "text-blue-400" },
    { label: "CRM sync status", sub: "All pipelines up to date", color: "text-green-400" },
  ];
  return (
    <div className="mockup-shell h-full min-h-[160px]">
      <div className="mockup-topbar">
        <div className="mockup-dot" /><div className="mockup-dot" /><div className="mockup-dot" />
      </div>
      <div className="p-2 sm:p-3 flex flex-col gap-1">
        {items.map((item) => (
          <div key={item.label} className="flex items-center justify-between px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg hover:bg-white/[0.03] transition-colors">
            <div>
              <p className="text-xs font-medium text-white/80 leading-tight">{item.label}</p>
              <p className="text-[10px] text-white/40 hidden sm:block">{item.sub}</p>
            </div>
            <div className={`w-4 h-4 ${item.color} opacity-70 flex-shrink-0`}>
...
function WorkflowMockup() {
  ...
  return (
    <div className="mockup-shell h-full min-h-[160px] p-3 sm:p-4">
      <div className="space-y-2">
        {steps.map((step, i) => (
          <div key={step} className="flex items-center gap-2">
            <div className={`w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-bold ${i < 3 ? 'bg-purple-600 text-white' : 'bg-white/10 text-white/30'}`}>{i + 1}</div>
            ...
            {i < 3 && <span className="text-purple-400 text-[10px] flex-shrink-0">✓</span>}
          </div>
        ))}
      </div>
    </div>
...
export default function Services() {
  ...
  return (
    <section id="services" className="relative py-16 sm:py-28 px-4 sm:px-6">
      <div className="relative max-w-5xl mx-auto">
        ...
        >
          ...
          <p className="mt-3 sm:mt-4 text-white/45 text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
        ...
        <div className="space-y-5 sm:space-y-8">
          {features.map((f, i) => (
            ...
            >
              {/* Text side */}
              <div className={`p-6 sm:p-8 md:p-10 flex flex-col justify-center gap-4 sm:gap-5 ${i % 2 === 1 ? "md:[direction:ltr]" : ""}`}>
                <span className="section-label w-fit text-xs">{f.tag}</span>
                <div>
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white tracking-tight leading-tight">
                    {f.title}
                  </h3>
                  <p className="mt-2 sm:mt-3 text-white/45 text-sm leading-relaxed">
                    {f.description}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {f.chips.map((chip) => (
                    <span key={chip} className="tag-chip text-xs">{chip}</span>
                  ))}
                </div>
                <a
                  href="#contact"
                  className="flex items-center gap-1.5 text-sm font-medium text-purple-400 hover:text-purple-300 transition-colors w-fit group min-h-[44px]"
                >
              ...
              {/* Mockup side */}
              <div className={`border-t md:border-t-0 ${i % 2 === 1 ? "md:border-r border-white/[0.07] md:[direction:ltr]" : "md:border-l border-white/[0.07]"} bg-black/40 p-4 sm:p-6 flex items-center`}>
